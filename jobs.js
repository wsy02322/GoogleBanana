...
export function createJobStore(rootDir) {
  ...
  function loadExisting() {
    ...
    for (const meta of entries.slice(-MAX_JOBS)) {
      jobs.set(meta.id, {
        ...meta,
        // Rehydrated jobs are not runnable; if they were mid-flight, mark error.
        status:
          meta.status === 'queued' || meta.status === 'running' ? 'error' : meta.status,
        error:
    ...
    // Drop older files beyond MAX_JOBS.
  ...
  async function runJob(job) {
    ...
    try {
      ...
      // Drop secrets before persisting anything else.
      delete job.headers
      delete job.body
      delete job.auth
  ...
  function createJob({ target, apiPath, headers, body, timeoutMs }) {
    const id = randomUUID()
    const now = Date.now()
    const job = {
      id,
    ...
    writeMeta(job)
    prune()
    // Detached runner — survives browser disconnect.
    setImmediate(() => {
  ...
  function getJob(id, { includeData = false } = {}) {
    const job = jobs.get(id)
    if (!job) return null
    return publicStatus(job, { includeData })
  }
